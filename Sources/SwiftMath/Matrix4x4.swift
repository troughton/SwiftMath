...
extension Matrix4x4f {
    ...
    public static let identity = Matrix4x4f(diagonal: Vector4f(repeating: 1.0))
    ...
    static func lookAt(eye: Vector3f, view: Vector3f, up: Vector3f) -> Matrix4x4f {
        ...
        return Matrix4x4f(
            Vector4f(right[0],        u[0],        view[0],        0.0),
            Vector4f(right[1],        u[1],        view[1],        0.0),
            Vector4f(right[2],        u[2],        view[2],        0.0),
            Vector4f(-dot(right, eye), -dot(u, eye), -dot(view, eye), 1.0)
        )
    }
    ...
    static func lookAtInv(eye: Vector3f, view: Vector3f, up: Vector3f) -> Matrix4x4f {
        ...
        return Matrix4x4f(
            Vector4f(right[0],          right[1],           right[2],        0.0),
            Vector4f(u[0],              u[1],               u[2],            0.0),
            Vector4f(view[0],           view[1],            view[2],         0.0),
            Vector4f(eye, 1.0)
        )
    }
    ...
    public static func lookAt(forward: Vector3f) -> Matrix4x4f {
        ...
        return Matrix4x4f(
            Vector4f(right[0],          right[1],           right[2],        0.0),
            Vector4f(u[0],              u[1],               u[2],            0.0),
            Vector4f(forward[0],        forward[1],         forward[2],      0.0),
    ...
    public static func scale(sx: Float, sy: Float, sz: Float) -> Matrix4x4f {
        return Matrix4x4f(diagonal: Vector4f(sx, sy, sz, 1.0))
    }
    ...
    public static func rotate(x: Angle, y: Angle) -> Matrix4x4f {
        ...
        return Matrix4x4f(
            Vector4f(cy,     0.0, sy,     0.0),
            Vector4f(sx*sy,  cx,  -sx*cy, 0.0),
            Vector4f(-cx*sy, sx,  cx*cy,  0.0),
            Vector4f(0.0,    0.0, 0.0,    1.0)
        )
    }
    ...
    public static func scaleRotateTranslate(sx _sx: Float, sy _sy: Float, sz _sz: Float,
        ...
        return Matrix4x4f(
            Vector4f(_sx * (cycz - sxsz*sy),   _sx * -cx*sz, _sx * (cz*sy + cy*sxsz), 0.0),
            Vector4f(_sy * (cz*sx*sy + cy*sz), _sy * cx*cz,  _sy * (sy*sz - cycz*sx), 0.0),
            Vector4f(_sz * -cx*sy,             _sz * sx,     cx*cy,                   0.0),
            Vector4f(tx,                       ty,           tz,                      1.0)
        )
    }
    ...
    public var decomposed : (translation: Vector3f, rotation: Quaternion, scale: Vector3f) {
        ...
        currentTransform[3] = Vector4f(0, 0, 0, 1)
        var scale = Vector3f(currentTransform[0].xyz.length, currentTransform[1].xyz.length, currentTransform[2].xyz.length)
...
extension Matrix4x4f {
    ...
    @inlinable
    public var translation : Vector4f {
        get {
            return self[3]
        }
        set {
